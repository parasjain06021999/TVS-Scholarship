...
export class MailService {
  ...
  async sendApplicationStatusEmail(email: string, firstName: string, status: string, scholarshipName: string) {
    const statusColors = {
      'SUBMITTED': '#3182ce',
      'UNDER_REVIEW': '#d69e2e',
      'APPROVED': '#38a169',
  ...
  async sendNotificationEmail(email: string, title: string, message: string, type: string) {
    const mailOptions = {
      from: this.configService.get('SMTP_FROM', 'noreply@tvsscholarship.com'),
      to: email,
  ...
  async sendEmail(emailData: { to: string; subject: string; text?: string; html?: string }) {
    const mailOptions = {
      from: this.configService.get('SMTP_FROM', 'noreply@tvsscholarship.com'),
      to: emailData.to,
