...
export class ChatbotService {
  ...
  public async generateBotResponse(userMessage: string, userId: string | null): Promise<string> {
    ...
    if (message.includes('application status') || message.includes('application')) {
      ...
      const statusSummary = userApplications.map(app => 
        `• ${app.scholarship.title}: ${app.status}`
      ).join('\n');
    ...
    if (message.includes('scholarship') || message.includes('scholarships')) {
      ...
      const scholarshipList = scholarships.map(sch => 
        `• ${sch.title} - ₹${sch.amount} (${sch.category})`
      ).join('\n');
    ...
    if (message.includes('deadline') || message.includes('last date')) {
      ...
      const deadlines = activeScholarships.map(sch => 
        `• ${sch.title}: ${new Date(sch.applicationEndDate).toLocaleDateString()}`
      ).join('\n');
